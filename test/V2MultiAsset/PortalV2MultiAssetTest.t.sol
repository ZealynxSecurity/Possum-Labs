...
contract PortalV2MultiAssetTest is Test {
    ...
    function setUp() public {
        ...
        /// MINT PSM TOKENS ///
        // Deploy the MintBurnToken contract representing PSM tokens
        MintBurnToken mintBurnPSM = new MintBurnToken("Peg-Stable-Mint", "PSM");

        // Store the address of the newly deployed MintBurnToken contract
        address mintBurnPSMAddress = address(mintBurnPSM);

        // Cast the MintBurnToken's address to the IERC20 interface for compatibility
        psm = IERC20(mintBurnPSMAddress);

        // Mint PSM tokens to psmSender address
        mintBurnPSM.mint(psmSender, psmAmount * 100_000);        
        uint256 balancePsmSender = psm.balanceOf(address(psmSender));
        ...
        /// MINT USDC TOKENS ///
        // Deploy the MintBurnToken contract representing USDC tokens
        MintBurnToken mintBurnUSDC = new MintBurnToken("USDC", "USDC");

        // Store the address of the newly deployed MintBurnToken contract
        address mintBurnUSDCAddress = address(mintBurnUSDC);

        // Cast the MintBurnToken's address to the IERC20 interface for compatibility
        usdc = IERC20(mintBurnUSDCAddress);

        // Mint PSM tokens to psmSender address
        mintBurnUSDC.mint(usdcSender, usdcAmount * 100_000);        
        uint256 balanceUsdcSender = usdc.balanceOf(address(usdcSender));
